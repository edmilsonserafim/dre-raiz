...
- Versão antiga e simplificada
- Não possui progress bar visual
- Não possui validação automática
- Não possui logs de erro detalhados
- Não gera chave_id única
- Não usa SQL Function (mais lento)
...
def conectar_fabric():
    """Conecta ao Microsoft Fabric usando Azure AD (ActiveDirectoryInteractive)"""
    print("[>] Autenticando no Azure AD...")

    # String de conexão com autenticação interativa nativa
    connection_string = (
    ...
    # Conectar diretamente (autenticação interativa)
