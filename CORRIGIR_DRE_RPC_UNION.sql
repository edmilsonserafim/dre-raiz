-- =====================================================
-- CORREÇÃO: DRE RPC com UNION das 3 tabelas
-- =====================================================
-- Data: 11/02/2026
...
AS $$
  ...
  WITH all_transactions AS (
    ...
    FROM transactions
    WHERE
      (p_month_from IS NULL OR date >= p_month_from || '-01')
...
BEGIN
  ...
  RETURN QUERY EXECUTE format(
    'WITH all_transactions AS (
       ...
       FROM transactions
       WHERE
         ($1 IS NULL OR date >= $1 || ''-01'')
         AND ($2 IS NULL OR date <= $2 || ''-31'')
...
-- RESULTADO ESPERADO
-- =====================================================
-- A query deve retornar dados de TODOS os 3 cenários:
-- ✅ Real (de transactions)
-- ✅ Orçado (de transactions_orcado)
-- ✅ A-1 (de transactions_ano_anterior)
--
-- ANTES: Retornava apenas "Real"
-- DEPOIS: Retorna "Real" + "Orçado" + "A-1"
-- =====================================================
